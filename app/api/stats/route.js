...
export async function GET(request) {
  try {
    ...
    // Revenue is attributed to the date the order was completed.
    let revenueDateFilter = '';
    const revenueDateParams = [];

    if (startDate && endDate) {
      revenueDateFilter = ` AND DATE(o.completed_at) BETWEEN ? AND ?`;
      revenueDateParams.push(startDate, endDate);
    } else if (dateRange === 'day') {
      revenueDateFilter = " AND DATE(o.completed_at) = DATE('now', 'localtime')";
    } else if (dateRange === 'yesterday') {
    ...
    const filteredRevenue = db
      .prepare(filteredRevenueSql)
      .get(...revenueDateParams).total;
